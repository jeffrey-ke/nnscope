...
from nnscope.tree import ModuleNode
...
def _summary(node: ModuleNode, annotations: dict[str, str]) -> str:
    ...
    if node.total_params:
        ...
        bits.append(f'<span class="params">{glyph} {_humanize(node.total_params)}{own}</span>')
...
def _render_node(
    ...
    summary = _summary(node, annotations)
    ...
    for child in node.children:
        _render_node(child, depth + 1, annotations, highlight, open_set, initial_depth, out)
    out.append("</details>")
...
def render_html(
    ...
    *,
    ...
    body: list[str] = []
    _render_node(root, 0, annotations, highlight, open_set, initial_depth, body)
